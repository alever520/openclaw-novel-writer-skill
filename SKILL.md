...
创作完成后：
- **6.0～6.6** 写入章节文件末尾，紧接正文，以如下分割线隔开：
  ```
  
  （第 X 章完）
  
  ```
- **6.7** 仅通过对话展示给用户，不写入文件。



**6.0 章节大纲回顾**

读取 `chapter-outlines/第XXX章-标题-大纲.md`（即步骤2确认并存档的章节大纲），附上完整内容（原文，不修改）：
...
> 在章节文件写入完成（含 6.0～6.6 追加到文件末尾）后，通过对话输出以下确认信息。
