...
```bash
# 读取项目的docs/writing-guide文件夹中的关键规范
# 至少包含: chapter-guide.md, confirmed-standards.md
```
...
✅ 已读取写作规范:
- docs/writing-guide/chapter-guide.md
...
- 遵守步骤3.5确认的章节要求和步骤4读取的写作指导与读者反馈
- 字数目标参照大纲中【预计字数】
...
创作完成后：
- **6.1～6.5** 写入章节文件末尾，紧接正文，以如下分割线隔开：
  ```
...
> 在章节文件写入完成（含 6.1～6.5 追加到文件末尾）后，通过对话输出以下确认信息（包含 6.6 字数统计）。
