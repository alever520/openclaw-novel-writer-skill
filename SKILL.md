...
⚠️ **重要**: 每次对正文做任何更新（新创作/修改/优化），都必须执行此循环。不是打标记，是直接修正正文。
...
[开始自检循环]
    ↓
第一轮: 对照 writing-guide.md
    ↓ 发现问题 → 立即修正正文 → 继续检查
    ↓ 全部通过 ↓
第二轮: 对照 characters.md
    ↓ 发现问题 → 立即修正正文 → 继续检查
    ↓ 全部通过 ↓
第三轮: 对照 reader-feedback.md
    ↓ 发现问题 → 立即修正正文 → 继续检查
    ↓ 全部通过 ↓
[提交 Git]
```

**第一轮：writing-guide.md 核查要点**

```
□ 语言风格
  - 无短句堆砌（连续3句以上单行短句）
  - 无宣言式独白（"这才是X""这不是X而是Y""我将要Z"）
  - 改法: 删结论句 → 替换为具体行动或细节反应
...
□ 章节结构
  - 无"第一幕/第二幕"等剧本标记
  - 世界观信息通过对话/行动自然展现，非旁白解说
  - 改法: 旁白解说 → 具体事件/对话承载信息

□ 节奏控制
  - 长段内心独白中间有外部事件打断
  - 食堂/街道等公共场景有具体小事发生（非纯旁白描述）
```

**第二轮：characters.md 核查要点**

```
□ 主角
  - 无标志性特征（动作习惯/口头禅不得作为塑造工具）
  - 行为模式：对话前有停顿、遇到无法理解的现象会"死盯"、欠人情记得清楚
  - 情绪展现真实：有落差感，不直接跳到结论状态

□ 配角（每人）
  - 标志性特征最多一个，且与背景相符
  - 改法: 多余标志性动作 → 删除或替换为一次性具体行为
  - 特征密度: 整章出现次数克制

□ 对话
  - 每个角色的语气/用词符合人设
  - 配角说话时有对应的肢体语言（仅在有戏份的场景）
```

**第三轮：reader-feedback.md 核查要点**
...
第一轮（writing-guide.md）:
  ✅ 语言风格: 无短句堆砌，无宣言式独白
  ⚠️ 场景转场: 第X段发现 --- 分割线 → 已替换为"[过渡句]"
  ✅ 章节结构: 无剧本标记，世界观自然展现
  ✅ 节奏: 内心独白有外部事件打断

第二轮（characters.md）:
  ⚠️ 主角: 发现2处"摸了摸下巴" → 已替换为具体行为
  ✅ 配角-托马斯: 仅1处低头眼神游移（符合标准）
  ✅ 对话: 各角色语气符合人设

第三轮（reader-feedback.md）:
  ✅ 反馈#3（宣言式独白）: 本章已无此问题
  ✅ 反馈#5（分割线）: 本章无章节内分割线
  ⚠️ 反馈#6（食堂叙事空转）: 发现食堂场景无具体事件 → 已加入[具体事件]
...
changelog:
  v1.5.0:
    - 重大新增: 正文自检循环（章节创作/修改的步骤4升级）
    - 核心机制: 每次对正文做更新时，必须依据 references/ 中的所有文档做一次自检循环，发现问题直接在正文中修正，不仅仅是打标记
    - 自检循环包含三轮:
      - 第一轮: 对照 references/writing-guide.md 逐条核查
      - 第二轮: 对照 references/characters.md 逐角色核查
      - 第三轮: 对照 references/reader-feedback.md 核查待处理问题
    - 每轮发现问题立即在正文中修正，修正后继续下一轮
    - 全部通过后方可提交 Git
