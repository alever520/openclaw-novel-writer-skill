...
**⚠️ 上下文管理**：
- **步骤1完成后可清空上下文**：全局大纲和上章预告仅用于生成章节大纲
- **原则**：一旦章节大纲确认并写入 `chapter-outlines/`，这些内容即可释放
...
**⚠️ 上下文管理**：
- **步骤2完成后可清空上下文**：人物卡仅用于生成章节大纲
- **原则**：大纲确认后释放，正文创作时按需重新读取（写到角色互动时临时读取）
...
⚠️ **读取的指导与反馈都需要严格遵守，在创作过程中全程应用。**

**⚠️ 上下文管理**：
- **步骤4完成后可清空上下文**：写作指导和读者反馈仅用于理解规则
- **原则**：理解规则后即可释放，正文创作时按规则执行（无需持续占用上下文）
...
- docs/writing-guide/confirmed-standards.md
...
**上下文要求（最小化策略）**：
- **必须保持**：当前章节大纲（`chapter-outlines/第XXX章-标题-大纲.md`）
- **按需读取**：
...
changelog:
  v2.1.0:
    - 新增: 上下文管理策略（最小化token消耗）
    - 步骤1、2、4完成后可清空上下文（全局大纲、人物卡、写作指导仅用于生成大纲和理解规则）
    - 步骤5正文创作时：必须保持章节大纲，按需临时读取人物卡/世界观（用完即弃）
    - 原则：章节大纲是唯一持续保持的上下文，其他内容"用时重读"
    - 明确标注：步骤1、2、4增加"⚠️ 上下文管理"说明
    - 步骤5修改：明确"上下文要求（最小化策略）"和"按需读取"规则
  v2.0.0:
