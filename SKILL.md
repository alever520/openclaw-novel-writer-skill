...
# 目的：从每个大类中提炼与本章相关的关键规则
```

**提炼原则**：
- **按大类提炼**：writing-guide.md 中的每个大类（如"章节结构准则"、"节奏控制原则"、"内容创作原则"等）各提炼1-3条
- **相关性判断**：根据步骤3确认的章节大纲，判断哪些规则与本章最相关
  - 例：本章有大量对话 → 从"对话创作"类提炼规则
  - 例：本章是开篇 → 从"开头技巧"类提炼规则
  - 例：本章有动作场景 → 从"节奏控制"类提炼规则
- **总量控制**：所有大类合计提炼不超过15条关键规则
...
**筛选原则**：
- **状态筛选**：只关注状态为"待处理"的条目
- **相关性判断**：与本章主题、场景、人物、节奏相关的反馈
  - 例：本章有主角独白 → 关注"独白过多"的反馈
  - 例：本章有新角色登场 → 关注"角色塑造"的反馈
- **无关反馈**：与本章无关的反馈不列出
...
✅ 已读取写作规范 (references/writing-guide.md)
...
✅ 已读取读者反馈 (references/reader-feedback.md)
...
**4.4 上下文管理**
- **汇报后可清空**：writing-guide.md 和 reader-feedback.md 的原文
- **保留内容**：提炼的关键规则（作为"理解"保留在上下文中）
- **创作执行**：步骤5正文创作时按提炼的规则执行，无需重新读取原文
...
**上下文管理说明**：
- 步骤3确认大纲后：步骤1（全局大纲、上章预告）和步骤2（人物卡）的原文已清空
- 步骤4汇报后：writing-guide.md 和 reader-feedback.md 的原文已清空
- **保留内容**：章节大纲 + 提炼的15条关键规则（约500字）
- **清空内容**：全局大纲、人物卡、完整写作规范、完整反馈记录（约10000字）
- **Token节省**：约95%（10000字 → 500字）

**创作规则**：
- 严格按步骤3确认的章节大纲推进剧情
- 场景顺序、节奏安排、伏笔处理均参照大纲执行
- 遵守步骤4提炼的关键规则（已在汇报中明确列出）
- 遵守步骤4筛选的待处理反馈（已在汇报中明确列出）
- 字数目标参照大纲中【预计字数】
...
创作完成后：
- **8.1～8.7** 写入章节文件末尾，紧接正文，以如下分割线隔开：
  ```
...
> 在章节文件写入完成（含 8.1～8.7 追加到文件末尾）后，通过对话输出以下确认信息（包含 8.6 字数统计）。
